...
const ImportData = () => {
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        ...
        <div className="grid grid-cols-3 gap-4">
          {importTypes.map(type => (
            <button key={type.id} onClick={() => { setSelectedType(type.id); setUploaded(false); }}
              className={`text-left p-4 rounded-xl border-2 transition-all ${selectedType === type.id ? 'border-blue-500 bg-blue-50' : 'border-gray-200 bg-white hover:border-blue-300 hover:bg-gray-50'}`}>
              <div className="flex items-center gap-3 mb-2">
                <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${selectedType === type.id ? 'bg-blue-100' : 'bg-gray-100'}`}>
      ...
      {selected && (
        <Card>
          <CardHeader><CardTitle className="flex items-center gap-2"><Icon name={selected.icon as never} size={20} className="text-blue-600" />Импорт: {selected.label}</CardTitle></CardHeader>
          <CardContent>
            <div className="grid grid-cols-2 gap-6">
              <div>
                <p className="text-sm font-medium text-gray-700 mb-2">Ожидаемые поля:</p>
                <div className="flex flex-wrap gap-2 mb-4">{selected.fields.map(f => <Badge key={f} className="bg-gray-100 text-gray-700">{f}</Badge>)}</div>
                <div className="flex gap-2">
                  <Button variant="outline" size="sm"><Icon name="Download" size={14} className="mr-1" />Шаблон Excel</Button>
                  <Button variant="outline" size="sm"><Icon name="FileText" size={14} className="mr-1" />Шаблон CSV</Button>
                </div>
              </div>
              <div>
                {!uploaded ? (
                  <div onDragOver={e => { e.preventDefault(); setDragOver(true); }} onDragLeave={() => setDragOver(false)} onDrop={e => { e.preventDefault(); setDragOver(false); setUploaded(true); }}
                    className={`border-2 border-dashed rounded-xl p-8 text-center transition-colors ${dragOver ? 'border-blue-500 bg-blue-50' : 'border-gray-300 hover:border-blue-400'}`}>
                    ...
                    <Button size="sm" onClick={() => setUploaded(true)}>Выбрать файл</Button>
                  ...
                  <div className="border-2 border-green-300 bg-green-50 rounded-xl p-6">
                    <div className="flex items-center gap-3 mb-4">
                      <div className="w-10 h-10 bg-green-100 rounded-full flex items-center justify-center"><Icon name="FileCheck" size={20} className="text-green-600" /></div>
                      <div><p className="font-medium text-gray-900">demo_import.xlsx</p><p className="text-sm text-gray-500">Найдено 87 строк данных</p></div>
                    </div>
                    <div className="space-y-1 text-sm mb-4">
                      <div className="flex justify-between"><span className="text-gray-600">Всего строк:</span><span className="font-medium">87</span></div>
                      <div className="flex justify-between"><span className="text-gray-600">Готово к импорту:</span><span className="font-medium text-green-600">85</span></div>
                      <div className="flex justify-between"><span className="text-gray-600">Ошибки/дубликаты:</span><span className="font-medium text-red-600">2</span></div>
                    </div>
                    <div className="flex gap-2">
                      <Button size="sm" className="flex-1"><Icon name="Upload" size={14} className="mr-1" />Импортировать (85)</Button>
                      <Button size="sm" variant="outline" onClick={() => setUploaded(false)}>Отмена</Button>
                    </div>
                  </div>
                )}
              </div>
        ...
        <CardContent>
          <table className="w-full text-sm">
            <thead><tr className="border-b text-left text-gray-500">
              <th className="pb-3 font-medium">ID</th><th className="pb-3 font-medium">Дата</th><th className="pb-3 font-medium">Тип данных</th>
              <th className="pb-3 font-medium">Файл</th><th className="pb-3 font-medium text-right">Строк</th><th className="pb-3 font-medium text-right">Успешно</th>
              <th className="pb-3 font-medium text-right">Ошибок</th><th className="pb-3 font-medium">Статус</th>
            </tr></thead>
            <tbody className="divide-y">
              {importHistory.map(h => (
                <tr key={h.id} className="hover:bg-gray-50">
                  <td className="py-3 text-gray-500">{h.id}</td><td className="py-3 text-gray-600">{h.date}</td>
                  <td className="py-3 font-medium">{h.type}</td><td className="py-3 text-gray-600">{h.file}</td>
                  <td className="py-3 text-right">{h.rows}</td><td className="py-3 text-right text-green-600 font-medium">{h.success}</td>
                  <td className="py-3 text-right text-red-600 font-medium">{h.errors > 0 ? h.errors : '—'}</td>
                  <td className="py-3"><Badge className={statusColor[h.status] || 'bg-gray-100 text-gray-700'}>{h.status}</Badge></td>
                </tr>
              ))}
            </tbody>
          </table>
