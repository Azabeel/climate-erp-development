...
const initialRoles = [
  { id: 'admin', name: 'Администратор', color: 'bg-red-100 text-red-700', users: 2, perms: defaultPerms(true) },
  {
    id: 'dispatcher', name: 'Диспетчер', color: 'bg-blue-100 text-blue-700', users: 5,
    perms: { ...defaultPerms(false),
  ...
  {
    id: 'engineer', name: 'Выездной инженер', color: 'bg-green-100 text-green-700', users: 12,
    perms: { ...defaultPerms(false),
      dashboard: { view: true, create: false, edit: false, delete: false },
  ...
  {
    id: 'manager', name: 'Менеджер продаж', color: 'bg-purple-100 text-purple-700', users: 3,
    perms: { ...defaultPerms(false),
      dashboard: { view: true, create: false, edit: false, delete: false },
  ...
  {
    id: 'accountant', name: 'Бухгалтер', color: 'bg-yellow-100 text-yellow-700', users: 1,
    perms: { ...defaultPerms(false),
...
const RolesConstructor = () => {
  ...
  const selectRole = (role: typeof initialRoles[0]) => { setSelectedRole(role); setPerms(role.perms); setSaved(false); };
  const toggle = (moduleId: string, perm: Permission) => {
    setPerms(prev => ({ ...prev, [moduleId]: { ...prev[moduleId], [perm]: !prev[moduleId]?.[perm] } }));
    setSaved(false);
  };
  const toggleAll = (moduleId: string) => {
    const allOn = permissions.every(p => perms[moduleId]?.[p]);
    setPerms(prev => ({ ...prev, [moduleId]: Object.fromEntries(permissions.map(p => [p, !allOn])) as Record<Permission, boolean> }));
    setSaved(false);
  };
      ...
      <div className="flex gap-6">
        <div className="w-64 shrink-0">
          <div className="flex items-center justify-between mb-3">
            ...
            <Button size="sm" variant="outline"><Icon name="Plus" size={14} className="mr-1" />Новая</Button>
          </div>
          <div className="space-y-2">
            {roles.map(role => (
              <button key={role.id} onClick={() => selectRole(role)}
                className={`w-full text-left p-3 rounded-lg border transition-all ${selectedRole.id === role.id ? 'border-blue-400 bg-blue-50' : 'border-gray-200 bg-white hover:bg-gray-50'}`}>
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  {saved && <div className="flex items-center gap-1 text-green-600 text-sm"><Icon name="CheckCircle" size={16} />Сохранено</div>}
                  <Button size="sm" onClick={() => setSaved(true)}><Icon name="Save" size={16} className="mr-1" />Сохранить</Button>
                </div>
              </div>
            </CardHeader>
            <CardContent>
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b text-left">
                    ...
                    {permissions.map(p => <th key={p} className="pb-3 font-medium text-gray-500 text-center">{permLabels[p]}</th>)}
                  </tr>
                </thead>
                      ...
                      <tr key={mod.id} className="hover:bg-gray-50">
                        ...
                        <td className="py-2.5 text-center">
                          <button onClick={() => toggleAll(mod.id)}
                            className={`w-5 h-5 rounded border-2 flex items-center justify-center mx-auto transition-colors ${allOn ? 'bg-blue-600 border-blue-600' : someOn ? 'bg-blue-200 border-blue-400' : 'border-gray-300'}`}>
                            {(allOn || someOn) && <Icon name="Check" size={12} className="text-white" />}
                          </button>
                        </td>
                        {permissions.map(perm => (
                          <td key={perm} className="py-2.5 text-center">
                            <button onClick={() => toggle(mod.id, perm)}
                              className={`w-5 h-5 rounded border-2 flex items-center justify-center mx-auto transition-colors ${modPerms[perm] ? 'bg-blue-600 border-blue-600' : 'border-gray-300 hover:border-blue-400'}`}>
