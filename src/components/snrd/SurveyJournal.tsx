...
const mockSurveys: CompletedSurvey[] = [
  ...
  { id: 'SRV-004', date: '10.05.2026', workOrderNumber: 'WO-2026-000106', clientName: 'Офис «ТехноГрупп»', objectName: 'Кондиционер LG S24EQ', templateName: 'Приёмка оборудования', templateType: 'Приёмка', engineer: 'Козлов Михаил', completionTime: 8, hasPhotos: true, hasSigning: true, score: 92, answers: [] },
  { id: 'SRV-005', date: '09.05.2026', workOrderNumber: 'WO-2026-000102', clientName: 'ТЦ «Садовод»', objectName: 'Кассетный кондиционер Gree', templateName: 'Рекламационный акт', templateType: 'Рекламация', engineer: 'Волков Николай', completionTime: 37, hasPhotos: true, hasSigning: true, score: 75, answers: [] },
  { id: 'SRV-006', date: '09.05.2026', workOrderNumber: 'WO-2026-000100', clientName: 'Ресторан «Причал»', objectName: 'Холодильная камера', templateName: 'Диагностика кондиционера', templateType: 'Диагностика', engineer: 'Иванов Иван', completionTime: 22, hasPhotos: false, hasSigning: false, score: 80, answers: [] },
  { id: 'SRV-007', date: '09.05.2026', workOrderNumber: 'WO-2026-000099', clientName: 'БЦ «Аэрополис»', objectName: 'Чиллер York YCSA', templateName: 'Плановое ТО (полное)', templateType: 'ТО', engineer: 'Новиков Андрей', completionTime: 92, hasPhotos: true, hasSigning: true, score: 97, answers: [] },
  { id: 'SRV-008', date: '08.05.2026', workOrderNumber: 'WO-2026-000095', clientName: 'Отель «Метрополь»', objectName: 'VRF-система Daikin VRVIII', templateName: 'Сдача после ремонта', templateType: 'Сдача', engineer: 'Петров Сергей', completionTime: 19, hasPhotos: true, hasSigning: true, score: 100, answers: [] },
  { id: 'SRV-009', date: '08.05.2026', workOrderNumber: 'WO-2026-000093', clientName: 'Завод «МашПром»', objectName: 'Приточная установка PU-10', templateName: 'Диагностика вентиляции', templateType: 'Диагностика', engineer: 'Морозов Денис', completionTime: 55, hasPhotos: true, hasSigning: false, score: 88, answers: [] },
  { id: 'SRV-010', date: '07.05.2026', workOrderNumber: 'WO-2026-000089', clientName: 'СК «Крылатское»', objectName: 'Кондиционер Haier AC60S', templateName: 'Приёмка оборудования', templateType: 'Приёмка', engineer: 'Козлов Михаил', completionTime: 11, hasPhotos: true, hasSigning: true, score: 94, answers: [] },
  { id: 'SRV-011', date: '07.05.2026', workOrderNumber: 'WO-2026-000087', clientName: 'Офис «ФинГрупп»', objectName: 'Кондиционер Panasonic CS-Z50TKEW', templateName: 'Плановое ТО (лайт)', templateType: 'ТО', engineer: 'Лебедев Артём', completionTime: 31, hasPhotos: true, hasSigning: true, score: 100, answers: [] },
  { id: 'SRV-012', date: '06.05.2026', workOrderNumber: 'WO-2026-000083', clientName: 'Бутик «Люкс»', objectName: 'Кондиционер Fujitsu ASYG12KETA', templateName: 'Диагностика кондиционера', templateType: 'Диагностика', engineer: 'Сидоров Алексей', completionTime: 18, hasPhotos: false, hasSigning: false, score: 72, answers: [] },
  { id: 'SRV-013', date: '06.05.2026', workOrderNumber: 'WO-2026-000081', clientName: 'ТЦ «Акварель»', objectName: 'Кассетный кондиционер Samsung', templateName: 'Плановое ТО (полное)', templateType: 'ТО', engineer: 'Волков Николай', completionTime: 67, hasPhotos: true, hasSigning: true, score: 96, answers: [] },
  { id: 'SRV-014', date: '05.05.2026', workOrderNumber: 'WO-2026-000078', clientName: 'Склад «Логистик»', objectName: 'Фанкойл Daikin FWF03ATN', templateName: 'Приёмка монтажа', templateType: 'Приёмка', engineer: 'Попов Виктор', completionTime: 23, hasPhotos: true, hasSigning: true, score: 99, answers: [] },
  { id: 'SRV-015', date: '05.05.2026', workOrderNumber: 'WO-2026-000075', clientName: 'Ресторан «Причал»', objectName: 'Вентустановка ВЕНТС ВУТ 300', templateName: 'Рекламационный акт', templateType: 'Рекламация', engineer: 'Иванов Иван', completionTime: 41, hasPhotos: true, hasSigning: true, score: 65, answers: [] },
];
      ...
      <Card>
        <CardContent className="pt-4">
          <div className="flex gap-3 flex-wrap">
            ...
            <select value={filterTemplate} onChange={(e) => setFilterTemplate(e.target.value)} className="px-3 py-2 border border-gray-300 rounded-md text-sm">
              <option value="all">Все шаблоны</option>
              {TEMPLATES.map((t) => <option key={t} value={t}>{t}</option>)}
            </select>
            <select value={filterEngineer} onChange={(e) => setFilterEngineer(e.target.value)} className="px-3 py-2 border border-gray-300 rounded-md text-sm">
      ...
      <Card>
        ...
        <CardContent>
          <Table>
            ...
            <TableBody>
              {filtered.map((survey) => (
                <TableRow key={survey.id} className="cursor-pointer hover:bg-gray-50" onClick={() => openDetail(survey)}>
                  <TableCell className="text-sm text-gray-700">{survey.date}</TableCell>
                  <TableCell><span className="text-xs font-mono text-blue-600">{survey.workOrderNumber}</span></TableCell>
                  <TableCell>
                  ...
                  <TableCell>
                    <div className="space-y-1">
                      ...
                      <Badge className={TYPE_COLORS[survey.templateType] + ' text-xs'}>{survey.templateType}</Badge>
                    </div>
                  </TableCell>
                  ...
                  <TableCell>
                    <div className="flex gap-1">
                      {survey.hasPhotos && <span title="Есть фото"><Icon name="Camera" size={15} className="text-blue-500" /></span>}
                      {survey.hasSigning && <span title="Есть подпись"><Icon name="PenTool" size={15} className="text-green-500" /></span>}
                    </div>
                  </TableCell>
                  <TableCell><Badge className={getScoreColor(survey.score)}>{survey.score}</Badge></TableCell>
                  <TableCell><Icon name="ChevronRight" size={16} className="text-gray-400" /></TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
          ...
          <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
            ...
            <div className="space-y-5">
              <div className="grid grid-cols-2 gap-3 bg-gray-50 rounded-lg p-4 text-sm">
                <div><span className="text-gray-400">Клиент:</span> <span className="font-medium">{selectedSurvey.clientName}</span></div>
                <div><span className="text-gray-400">Объект:</span> <span className="font-medium">{selectedSurvey.objectName}</span></div>
                <div><span className="text-gray-400">Инженер:</span> <span className="font-medium">{selectedSurvey.engineer}</span></div>
                <div><span className="text-gray-400">Дата:</span> <span className="font-medium">{selectedSurvey.date}</span></div>
                <div><span className="text-gray-400">Шаблон:</span> <span className="font-medium">{selectedSurvey.templateName}</span></div>
                <div><span className="text-gray-400">Время заполнения:</span> <span className="font-medium">{selectedSurvey.completionTime} мин</span></div>
              </div>
              <div className="flex items-center gap-3">
                <span className="text-sm text-gray-500">Итоговый балл:</span>
                <Badge className={`text-lg px-3 py-1 ${getScoreColor(selectedSurvey.score)}`}>{selectedSurvey.score} / 100</Badge>
              </div>
                ...
                <div className="space-y-3">
                  ...
                  {selectedSurvey.answers.map((a, idx) => (
                    <div key={idx} className="border border-gray-200 rounded-lg p-3">
                      <div className="text-xs text-gray-400 mb-1">{idx + 1}. {a.question}</div>
                      {a.type === 'Фото' ? (
                        <div className="grid grid-cols-3 gap-2 mt-2">
                          {[1,2,3].map((n) => (
                            <div key={n} className="aspect-square bg-gray-100 rounded-lg flex items-center justify-center border border-gray-200">
            ...
            <DialogFooter>
              <Button variant="outline"><Icon name="Download" size={15} className="mr-2" />Скачать PDF</Button>
