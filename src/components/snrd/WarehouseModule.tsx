  ...
  return (
    <div className="p-6">
      <div className="grid grid-cols-4 gap-4 mb-6">
        <Card><CardContent className="pt-4"><div className="flex items-center justify-between"><div><p className="text-sm text-gray-500">Позиций на складе</p><p className="text-2xl font-bold">{stockItems.length}</p></div><div className="w-10 h-10 bg-blue-100 rounded-lg flex items-center justify-center"><Icon name="Package" size={20} className="text-blue-600" /></div></div></CardContent></Card>
        <Card><CardContent className="pt-4"><div className="flex items-center justify-between"><div><p className="text-sm text-gray-500">Стоимость склада</p><p className="text-2xl font-bold">{(totalValue / 1000).toFixed(0)}K ₽</p></div><div className="w-10 h-10 bg-green-100 rounded-lg flex items-center justify-center"><Icon name="CircleDollarSign" size={20} className="text-green-600" /></div></div></CardContent></Card>
        <Card><CardContent className="pt-4"><div className="flex items-center justify-between"><div><p className="text-sm text-gray-500">Баллонов хладагента</p><p className="text-2xl font-bold">{cylinders.length}</p></div><div className="w-10 h-10 bg-cyan-100 rounded-lg flex items-center justify-center"><Icon name="Wind" size={20} className="text-cyan-600" /></div></div></CardContent></Card>
        <Card><CardContent className="pt-4"><div className="flex items-center justify-between"><div><p className="text-sm text-gray-500">Критический запас</p><p className="text-2xl font-bold text-red-600">{lowStock}</p></div><div className="w-10 h-10 bg-red-100 rounded-lg flex items-center justify-center"><Icon name="AlertTriangle" size={20} className="text-red-600" /></div></div></CardContent></Card>
      </div>
      <Tabs defaultValue="stock">
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  ...
                  <select value={catFilter} onChange={e => setCatFilter(e.target.value)} className="px-3 py-2 border rounded-md text-sm">
                    {categories.map(c => <option key={c} value={c}>{c === 'all' ? 'Все категории' : c}</option>)}
                  </select>
                  <Button size="sm"><Icon name="Plus" size={16} className="mr-1" />Оприходовать</Button>
                </div>
              </div>
            </CardHeader>
            <CardContent>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead><tr className="border-b text-left text-gray-500">
                    <th className="pb-3 font-medium">Артикул</th><th className="pb-3 font-medium">Наименование</th><th className="pb-3 font-medium">Категория</th>
                    <th className="pb-3 font-medium text-right">Кол-во</th><th className="pb-3 font-medium text-right">Резерв</th><th className="pb-3 font-medium text-right">Доступно</th>
                    <th className="pb-3 font-medium">Склад</th><th className="pb-3 font-medium text-right">Цена</th><th className="pb-3 font-medium text-right">Сумма</th>
                  </tr></thead>
                    ...
                    {filtered.map(item => {
                      ...
                      return (
                        <tr key={item.id} className="hover:bg-gray-50">
                          ...
                          <td className="py-3 font-medium"><div className="flex items-center gap-2">{item.name}{isLow && <Badge className="bg-red-100 text-red-700 text-xs">Мало</Badge>}</div></td>
                  ...
                  <tfoot><tr className="border-t font-semibold bg-gray-50"><td colSpan={8} className="py-3 text-right pr-4">Итого:</td><td className="py-3 text-right">{totalValue.toLocaleString()} ₽</td></tr></tfoot>
                </table>
              </div>
            </CardContent>
          </Card>
        ...
        <TabsContent value="movements">
          <Card>
            <CardHeader><div className="flex items-center justify-between"><CardTitle>Движение товаров</CardTitle><div className="flex gap-2"><Input placeholder="Поиск..." value={movSearch} onChange={e => setMovSearch(e.target.value)} className="w-48" /><Button size="sm"><Icon name="Plus" size={16} className="mr-1" />Новое движение</Button></div></div></CardHeader>
            <CardContent>
              <table className="w-full text-sm">
                <thead><tr className="border-b text-left text-gray-500">
                  <th className="pb-3 font-medium">Дата</th><th className="pb-3 font-medium">Тип</th><th className="pb-3 font-medium">Номенклатура</th>
                  <th className="pb-3 font-medium text-right">Кол-во</th><th className="pb-3 font-medium">Откуда</th><th className="pb-3 font-medium">Куда</th>
                  <th className="pb-3 font-medium">Сотрудник</th><th className="pb-3 font-medium text-right">Сумма</th>
                </tr></thead>
                <tbody className="divide-y">
                  {filteredMov.map(m => (
                    <tr key={m.id} className="hover:bg-gray-50">
                      <td className="py-3 text-gray-500">{m.date}</td>
                      <td className="py-3"><Badge className={movTypeColor[m.type] || 'bg-gray-100 text-gray-700'}>{m.type}</Badge></td>
                      <td className="py-3 font-medium">{m.item}</td>
                      <td className={`py-3 text-right font-medium ${m.qty < 0 ? 'text-red-600' : 'text-green-600'}`}>{m.qty > 0 ? '+' : ''}{m.qty}</td>
        ...
        <TabsContent value="refrigerants">
          <div className="grid grid-cols-2 gap-6">
            <Card>
              <CardHeader><div className="flex items-center justify-between"><CardTitle>Баллоны хладагента</CardTitle><Button size="sm"><Icon name="Plus" size={16} className="mr-1" />Добавить баллон</Button></div></CardHeader>
              <CardContent>
                <table className="w-full text-sm">
                  <thead><tr className="border-b text-left text-gray-500"><th className="pb-3 font-medium">ID</th><th className="pb-3 font-medium">Тип</th><th className="pb-3 font-medium">Серийный №</th><th className="pb-3 font-medium text-right">Остаток</th><th className="pb-3 font-medium">Статус</th></tr></thead>
                  <tbody className="divide-y">
                    {cylinders.map(c => {
                      ...
                      return (
                        <tr key={c.id} className="hover:bg-gray-50">
                          ...
                          <td className="py-2 text-right"><div className="flex items-center justify-end gap-2"><div className="w-16 bg-gray-200 rounded-full h-1.5"><div className={`h-1.5 rounded-full ${pct < 20 ? 'bg-red-500' : pct < 50 ? 'bg-yellow-500' : 'bg-green-500'}`} style={{ width: `${pct}%` }} /></div><span className="text-xs">{c.remaining}/{c.capacity} кг</span></div></td>
                          <td className="py-2"><Badge className={c.status === 'Активен' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}>{c.status}</Badge></td>
                        </tr>
                      );
                    })}
                  </tbody>
            ...
            <Card>
              <CardHeader><CardTitle>Журнал операций с хладагентами</CardTitle></CardHeader>
              <CardContent>
                <table className="w-full text-sm">
                  <thead><tr className="border-b text-left text-gray-500"><th className="pb-3 font-medium">Дата</th><th className="pb-3 font-medium">Операция</th><th className="pb-3 font-medium">Хладагент</th><th className="pb-3 font-medium text-right">кг</th><th className="pb-3 font-medium">Инженер</th></tr></thead>
                  <tbody className="divide-y">
                    {refrigerantLog.map(r => (
                      <tr key={r.id} className="hover:bg-gray-50">
                        <td className="py-2 text-gray-500">{r.date}</td>
                        <td className="py-2"><Badge className={refTypeColor[r.type] || 'bg-gray-100 text-gray-700'}>{r.type}</Badge></td>
                ...
                <div className="mt-4 p-3 bg-amber-50 border border-amber-200 rounded-lg"><div className="flex items-center gap-2 text-amber-800 text-sm"><Icon name="AlertTriangle" size={14} /><span className="font-medium">Внимание:</span><span>1 единица оборудования превышает норму утечки (32% &gt; 30%)</span></div></div>
              </CardContent>
            </Card>
          </div>
        </TabsContent>
